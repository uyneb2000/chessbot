...
public final class Board extends Canvas {
	...
	public void setBoard(String initial) {
		int row, col;

		if ("*".equals(initial))
			initial = initialPosition;
		
		String[] board = initial.split(" ")[0].split("/");
		
		for (int i = 0; i < board.length; i++) {
		    int jPieces = 0;
		    for (int j = 0; j < board[i].length(); j++) {
		        if ("12345678".contains("" + board[i].charAt(j))) { 
		            int num = board[i].charAt(j) - '0';
		            while (num > 0) {
		                pieces[i][jPieces++] = '-';
		                num--;
		            }
		            ...
		            pieces[i][jPieces++] = board[i].charAt(j);
		        }
		    }
		}
	}
	...
	private void setStateVariables(String move) {
		String algFrom = move.substring(0, 2);
		Point bdFrom = algebraicToBoard(algFrom);
		String algTo = move.substring(2, 4);
	...
	public void setHighlights(String move) {
		if (move == null) {
			highlit1 = null;
			highlit2 = null;
			return;
		...
		highlit1 = algebraicToBoard(move.substring(0, 2));
		highlit2 = algebraicToBoard(move.substring(2, 4));
	}
	...
	public boolean mouseDown(Event evt, int x, int y) {
		if (Config.boolGuest) {
			...
			if (x < 8 * squareSize && y < 8 * squareSize) {
				...
				highlit1 = mouseFrom;
				highlit2 = null;
				// repaint(//hilit1.x, hilit1.y, squareSize, squareSize//);
			} 
			else {
				mouseFrom = null;
			}
	...
	public boolean mouseDrag(Event evt, int x, int y) {
		if (Config.boolGuest) {
			...
			if (!p2.equals(mouseTo)) {
				mouseTo = p2;
				if (isLegalMove(mouseFrom, mouseTo)) {
			...
			if (x < 8 * squareSize && y < 8 * squareSize) {
				...
				if (algFrom.equals(algTo)) {
					...
					if ((p2.y == 0 || p2.y == 7)
						...
						hub.sendCommand(algFrom + algTo + '=' + hub.getPromote());
					} 
					else {
						hub.sendCommand(algFrom + algTo);
					}
				}
			}
			repaint();
	...
	/*private void doSmithMove(String move) {
		...
		setHighlights(move);
		setStateVariables(move);
		// When a new move is applied, no longer need to show the best move
		// found during the search
		setBestMove(null);

		from = algebraicToBoard(move.substring(0, 2));
		to = algebraicToBoard(move.substring(2, 4));
		piece = getPiece(from);
		
		char end = move.charAt(len - 1);
		...
		// shouldn't get here
		System.out.println("bogus smith move: " + move);
	}*/
