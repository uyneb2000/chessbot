...
public class ArrayBoard implements Board<ArrayMove,ArrayBoard>
{
  ...
  public void    applyMove(ArrayMove move)
  {
  	...
  	UndoMove   undo         = UndoMove.FACTORY.create().init(this, move);
  ...
  private void makeMoves(ArrayPiece source, List<ArrayPiece> dests, List<ArrayMove> accumulator)
  {
  	...
  	{
  		ArrayMove m = ArrayMove.FACTORY.create().init(source, dest, dest.isOccupied());
  		accumulator.add(m);
  	}
  }
  ...
  private void makeCastlingMoves(ArrayPiece source, int color, List<ArrayMove> accumulator)
  {
      ...
      if(color == WHITE)
      {
          ...
          {
              ArrayMove m = ArrayMove.FACTORY.create().init(source,board[G1],false);
              accumulator.add(m);
          }
          ...
          {
              ArrayMove m = ArrayMove.FACTORY.create().init(source,board[C1],false);
              accumulator.add(m);
          }
      }
      else
      {
          ...
          {
              ArrayMove m = ArrayMove.FACTORY.create().init(source,board[G8],false);
              accumulator.add(m);
          }
          ...
          {
              ArrayMove m = ArrayMove.FACTORY.create().init(source,board[C8],false);
              accumulator.add(m);
          }
      }
  }
  ...
  private void makeEnpassants(ArrayPiece source, List<ArrayMove> accumulator)
  {
      ...
      {
          ArrayMove m = ArrayMove.FACTORY.create().init(source,dest,true);
          accumulator.add(m);
      }
  }
  
  /**
   * Fills an accumulator with all of the pieces which are enpassant attack
   * by the given square.
   * 
      ...
      for(ArrayPiece dest : dests)
      {
          ...
          if(drow == 0 || drow == 7)
          {
              for(int promote : PROMOTED_PIECES[source.color()])
              {
                  ...
                  ArrayMove  m  = ArrayMove.FACTORY.create().init(source, dest, pr, dest.isOccupied());
                  accumulator.add(m);
              }
              promotions.add(dest);
          }
  ...
  public boolean isLegalMove(ArrayMove move)
  {
    ...
    if( move.isCapture() )
    {
      if( srcpiece.color() == destpiece.color() && destpiece.isOccupied() )
      {
        return false;
      }
    }
